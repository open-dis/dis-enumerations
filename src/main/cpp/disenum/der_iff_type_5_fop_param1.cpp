#include <sstream>
#include <cstddef>
#include <disenum/der_iff_type_5_fop_param1.h>
...
namespace der_iff_type_5_fop_param1 {
  ...
  void bitmask::setValue(const unsigned short& i){
	...
	hashMap<int,Code*> Code::enumerations;
	short Code::startBit = 0;
	short Code::endBit   = 4;



	Code::Code(int value, std::string description) :
	  Enumeration(value, description)
	{
	  enumerations[value] = this;
	};

	Code* Code::findEnumeration(int aVal) {
	  Code* pEnum;
	...
	std::string Code::getDescriptionForValue(int aVal) {
	  Code* pEnum = findEnumeration(aVal);
	...
	Code Code::getEnumerationForValue(int aVal) throw(EnumException) {
	  Code* pEnum = findEnumeration(aVal);
	  if (pEnum) return (*pEnum);
	  else  {
		std::stringstream ss;
		ss << "No enumeration found for value " << aVal << " of enumeration Code";
		throw EnumException("Code", aVal, ss.str());
	  }
	};

	bool Code::enumerationForValueExists(int aVal) {
	  Code* pEnum = findEnumeration(aVal);
	  if (pEnum) return (true);
	  else       return (false);
	};

	Code::enumContainer Code::getEnumerations() {
	  return (enumerations);
	};


	/*
	 **  PowerReduction implementation **
	 */

	hashMap<int,PowerReduction*> PowerReduction::enumerations;
	short PowerReduction::startBit = 11;
	short PowerReduction::endBit   = 11;

 PowerReduction PowerReduction::OFF(0, "Off");
 PowerReduction PowerReduction::ON(1, "On");


	PowerReduction::PowerReduction(int value, std::string description) :
	  Enumeration(value, description)
	{
	  enumerations[value] = this;
	};

	PowerReduction* PowerReduction::findEnumeration(int aVal) {
	  PowerReduction* pEnum;
	...
	std::string PowerReduction::getDescriptionForValue(int aVal) {
	  PowerReduction* pEnum = findEnumeration(aVal);
	  if (pEnum) return pEnum->description;
	  else {
		std::stringstream ss;
		ss <<  "Invalid enumeration: " << aVal;
	...
	PowerReduction PowerReduction::getEnumerationForValue(int aVal) throw(EnumException) {
	  PowerReduction* pEnum = findEnumeration(aVal);
	  if (pEnum) return (*pEnum);
	  else  {
		std::stringstream ss;
		ss << "No enumeration found for value " << aVal << " of enumeration PowerReduction";
		throw EnumException("PowerReduction", aVal, ss.str());
	  }
	};

	bool PowerReduction::enumerationForValueExists(int aVal) {
	  PowerReduction* pEnum = findEnumeration(aVal);
	  if (pEnum) return (true);
	  else       return (false);
	};

	PowerReduction::enumContainer PowerReduction::getEnumerations() {
	  return (enumerations);
	};


	/*
	 **  RadarEnhancement implementation **
	 */

	hashMap<int,RadarEnhancement*> RadarEnhancement::enumerations;
	short RadarEnhancement::startBit = 12;
	short RadarEnhancement::endBit   = 12;

 RadarEnhancement RadarEnhancement::OFF(0, "Off");
 RadarEnhancement RadarEnhancement::ON(1, "On");


	RadarEnhancement::RadarEnhancement(int value, std::string description) :
	  Enumeration(value, description)
	{
	  enumerations[value] = this;
	};

	RadarEnhancement* RadarEnhancement::findEnumeration(int aVal) {
	  RadarEnhancement* pEnum;
	...
	std::string RadarEnhancement::getDescriptionForValue(int aVal) {
	  RadarEnhancement* pEnum = findEnumeration(aVal);
	  if (pEnum) return pEnum->description;
	  else {
		std::stringstream ss;
		ss <<  "Invalid enumeration: " << aVal;
	...
	RadarEnhancement RadarEnhancement::getEnumerationForValue(int aVal) throw(EnumException) {
	  RadarEnhancement* pEnum = findEnumeration(aVal);
	  if (pEnum) return (*pEnum);
	  else  {
		std::stringstream ss;
		ss << "No enumeration found for value " << aVal << " of enumeration RadarEnhancement";
		throw EnumException("RadarEnhancement", aVal, ss.str());
	  }
	};

	bool RadarEnhancement::enumerationForValueExists(int aVal) {
	  RadarEnhancement* pEnum = findEnumeration(aVal);
	  if (pEnum) return (true);
	  else       return (false);
	};

	RadarEnhancement::enumContainer RadarEnhancement::getEnumerations() {
	  return (enumerations);
	};
