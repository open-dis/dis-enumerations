#include <sstream>
#include <cstddef>
#include <disenum/der_iff_type_2_fop_param6.h>
...
namespace der_iff_type_2_fop_param6 {
  ...
  void bitmask::setValue(const unsigned short& i){
	...
	hashMap<int,TCAS*> TCAS::enumerations;
	short TCAS::startBit = 12;
	short TCAS::endBit   = 12;

 TCAS TCAS::TCAS_I(0, "TCAS I");
 TCAS TCAS::TCAS_II(1, "TCAS II");


	TCAS::TCAS(int value, std::string description) :
	  Enumeration(value, description)
	{
	  enumerations[value] = this;
	};

	TCAS* TCAS::findEnumeration(int aVal) {
	  TCAS* pEnum;
	...
	std::string TCAS::getDescriptionForValue(int aVal) {
	  TCAS* pEnum = findEnumeration(aVal);
	...
	TCAS TCAS::getEnumerationForValue(int aVal) throw(EnumException) {
	  TCAS* pEnum = findEnumeration(aVal);
	  if (pEnum) return (*pEnum);
	  else  {
		std::stringstream ss;
		ss << "No enumeration found for value " << aVal << " of enumeration TCAS";
		throw EnumException("TCAS", aVal, ss.str());
	  }
	};

	bool TCAS::enumerationForValueExists(int aVal) {
	  TCAS* pEnum = findEnumeration(aVal);
	  if (pEnum) return (true);
	  else       return (false);
	};

	TCAS::enumContainer TCAS::getEnumerations() {
	  return (enumerations);
	};
